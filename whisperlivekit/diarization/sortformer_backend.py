...
class SortformerDiarization:
    def __init__(self, model_name: str = "nvidia/diar_streaming_sortformer_4spk-v2", gpu_id: int | None = None):
        """
        Stores the shared streaming Sortformer diarization model. Used when a new online_diarization is initialized.
        
        Args:
            model_name: Hugging Face model name
            gpu_id: Specific GPU device ID. If None, uses default CUDA device.
        """
        self.gpu_id = gpu_id
    ...
    def _load_model(self, model_name: str):
        ...
        try:
            ...
            # Use specified GPU or default
            if self.gpu_id is not None and torch.cuda.is_available():
                if self.gpu_id >= torch.cuda.device_count():
                    logger.warning(f"GPU {self.gpu_id} not available for Sortformer, using GPU 0")
                    device = torch.device("cuda:0")
                else:
                    device = torch.device(f"cuda:{self.gpu_id}")
                logger.info(f"Sortformer using GPU {self.gpu_id}")
            elif torch.cuda.is_available():
                device = torch.device("cuda")
                logger.info("Sortformer using default CUDA device")
            else:
                device = torch.device("cpu")
                logger.warning("Sortformer using CPU")
